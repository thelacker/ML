...
new_X_test = list()
for x in X_test:
    proba = (classifiers[0].predict_proba(x) + classifiers[1].predict_proba(x) +
             classifiers[2].predict_proba(x) + classifiers[3].predict_proba(x))/4
...
all = 0
for x in new_X_test:
    if x[0][0] >= 0.7 and y_test[i] == 0:
        true += 1
    if x[0][0] < 0.7 and y_test[i] == 1:
        true += 1
    all += 1
    i += 1
